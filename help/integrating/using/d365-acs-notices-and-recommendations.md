---
title: Campaign とMicrosoft Dynamics 365 データ管理
...
>[!IMPORTANT]
>
>**双方向** または **一方向（Campaign からMicrosoft Dynamics 365）** のオプトアウト設定が必要な場合、Campaign インスタンスにオプトアウトワークフローを設定するには、Adobeの技術担当者にリクエストを送信する必要があります
...
| ユースケース | 説明 |
|---|---|
| 双方向および単方向（Campaign からMicrosoft Dynamics 365） | 双方向および一方向（Campaign からMicrosoft Dynamics 365）のオプトアウトデータフローでは、Campaign SFTP ストレージを使用します。 Campaign ワークフローでは、増分変更を SFTP フォルダーにエクスポートします。 そこから、統合はレコードとプロセスを取得します。 |
...
>[!IMPORTANT]
>
>ユーザー側の特定のアクション（レコードの初期取り込み、レコードデータの再生など） その結果、大量のレコードがMicrosoft Dynamics 365 からAdobe Campaign インスタンスに取り込まれる可能性があります。 パフォーマンス上の問題が発生するリスクを軽減するために、すべての Campaign プロセスを停止することをお勧めします（マーケティングアクティビティやワークフローの実行など）。 大量のレコードが Campaign に取り込まれるまで。

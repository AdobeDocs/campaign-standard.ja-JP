...
description: Adobe Campaign Standardで電子メールチャネルを設定する方法について説明します
...
Campaign [管理者](../../administration/using/users-management.md#functional-administrators)は、メールチャネルを設定できます。 これらの詳細設定には、一般的なメールチャネルパラメーター、メールルーティングアカウント、メール処理ルールおよびメールのプロパティが含まれます。 このページでは、一般的なメールと送信パラメーターのデフォルト値を編集する方法について説明します。
...
* **許可されたマスクフィールド**
  ...
  **[!UICONTROL Header parameters of sent emails]** セクションには、受信者に電子メールを送信するために使用できる承認済み電子メールアドレス（送信者アドレス）と、非同期バウンスや不在時の返信などの自動返信（エラーアドレス）を受信者が送信できるようにするための承認済み電子メールアドレスがリストされます。

  Adobe Campaign は、メッセージの準備段階で、入力されたアドレスが有効であるかどうかを確認します。 この動作モードでは、配信品質の問題をトリガーする可能性のあるアドレスを一切使用しないようにします。

   * 送信者アドレスとエラーアドレスの両方は Adobe が設定します。 これらのフィールドは空にできません。
   * これらのフィールドは編集できません。 住所を更新する場合は、Adobe カスタマーケアチームにお問い合わせください。
   * 別のアドレスを追加するには、[&#x200B; キャンペーンCampaign コントロールパネル](https://experienceleague.adobe.com/docs/control-panel/using/subdomains-and-certificates/setting-up-new-subdomain.html?lang=ja)を使用して新しいサブドメインを設定するか、Adobe カスタマーケアチームにお問い合わせください。 複数のマスクを使用する場合は、コンマで区切ります。
   * **@yourdomain.com**&#x200B;などの星を使用してアドレスを設定することをお勧めします。これにより、サブドメイン名で終わるすべてのアドレスを使用できます。
...
* **配信パラメーター**
  ...
  Adobe Campaignは、開始日からメッセージを送信します。

  一時的なエラーまたはソフトバウンスが原因で配信内のメッセージが拒否された場合、Campaignは毎日このメッセージを再送信します。 **[!UICONTROL Message delivery duration]** フィールドを使用して、再試行中に発生する可能性のある期間を指定します。

  >[!IMPORTANT]
  >
  >**Campaignのこのパラメーターは、3.5日以内に設定された場合にのみ使用されるようになりました。** 3.5 日を超える値を定義した場合、その値は考慮されません。
...
* **再試行**
  ...
  >[!IMPORTANT]
  >
  >実行される再試行の最大数と再試行間の最小遅延は、IPが特定のドメインで過去と現在の両方をどの程度実行しているかに基づいています。 Campaignの&#x200B;**[!UICONTROL Retry period]**&#x200B;と&#x200B;**[!UICONTROL Number of retries]**&#x200B;の設定は無視されます。
...
* **メール強制隔離のパラメーター**
  ...
  **[!UICONTROL Maximum number of errors before quarantine]**&#x200B;値に達すると、メールアドレスが隔離されます。 デフォルト値は&#x200B;**&quot;5&quot;**&#x200B;です。アドレスは5番目のエラーで強制隔離されます。 これは、連絡先が後続の配信から自動的に除外されることを意味します。
...
>[!IMPORTANT]
>
>メールドメインとMX ルールは自動的に管理されるようになりました<!--by the Adobe Campaign Enhanced MTA (Message Transfer Agent)-->。変更できません。

* **DKIM （DomainKeys Identified Mail）**&#x200B;すべてのドメインを持つすべてのメッセージに対して、メール認証の署名が行われます。 **送信者ID**、**DomainKeys**、または&#x200B;**S/MIME**&#x200B;で署名しません。
* MX ルールは、過去の電子メールのレピュテーション履歴と、メールを送信するドメインからのリアルタイムフィードバックにもとづいて、スループットをドメインごとに自動的にカスタマイズします。
...
>[!IMPORTANT]
>
>同期配信エラーのエラーメッセージは、バウンスの種類と選定を決定し、その情報をCampaignに送り返すAdobe Campaign強化MTAによって選定されるようになりました。
...
「**[!UICONTROL Campaign]**」フィールドに、メールに関連付けられているキャンペーンを入力します。
...
#### パラメーターを再試行 {#retries-parameters}

一時的に配信できなかったメッセージは、自動再試行の対象となります。 詳しくは、[一時的な配信エラーの後の再試行](../../sending/using/understanding-delivery-failures.md#retries-after-a-delivery-temporary-failure)を参照してください。

>[!IMPORTANT]
>
>再試行と再試行の最大数との間の最小の遅延は、IPが特定のドメインで過去と現在の両方をどの程度実行しているかに基づいています。 Campaignの&#x200B;**[!UICONTROL Retry period]**&#x200B;と&#x200B;**[!UICONTROL Max. number of retries]**&#x200B;の設定は無視されます。
...
#### メール形式パラメーター {#email-format-parameters}
...
実際にメッセージを送信せずにSMTP接続を介してメール送信をテストするには、**[!UICONTROL Enable SMTP test mode]** オプションを使用します。 配信は SMTP サーバーに接続するまで処理されますが、送信はされません。配信のすべての受信者について、Campaign は SMTP プロバイダーサーバーに接続し、「SMTP RCPT TO」コマンドを実行して、「SMTP DATA」コマンドの前に接続を閉じます。
...
### 有効期限パラメーター {#validity-period-parameters}
...
* **[!UICONTROL Delivery duration]**／**[!UICONTROL Validity limit for sending messages]**：Adobe Campaign は、開始日に開始するメッセージを送信します。 このフィールドを使用して、メッセージを送信できる期間を指定します。
  ...
  >[!IMPORTANT]
  >
  >**3.5日までの値を定義する必要があります。** 3.5日を超える値を設定した場合、その値は考慮されません。
  >
...
* **[!UICONTROL Mirror page management]**：ミラーページは、Web ブラウザーからオンラインアクセス可能な HTML ページです。 コンテンツはメールの内容と変わりません。 デフォルトでは、メールコンテンツ内にリンクが挿入されているとミラーページが生成されます。 このフィールドを使用して、このページの生成方法を変更します。
  ...
  >[!IMPORTANT]
  >
  >ミラーページは、メールにHTML コンテンツが定義されている場合にのみ生成されます。
  >
...
* **[!UICONTROL Activate tracking]**：このオプションを使用して、メッセージ URL トラッキングをアクティブ化/非アクティブ化します。 各メッセージ URL の追跡を管理するには、E メールデザイナーのアクションバーにある「**[!UICONTROL Links]**」アイコンを使用します。 [追跡する URL について](../../designing/using/links.md#about-tracked-urls)を参照してください。
* **[!UICONTROL Tracking validity limit]**：このオプションを使用して、トラッキングがURLでアクティブ化される期間を定義します。
* **[!UICONTROL Substitution URL for expired URLs]**：このオプションを使用して、フォールバック web ページのURLを入力します。トラッキングの有効期限が切れると表示されます。
* **[!UICONTROL Use tracking pixel at the top of email]**：このオプションを使用すると、下部ではなくメールの上部にトラッキングピクセルを移動できます。 デフォルトでは、このピクセルはメールの下部にあります。 大きなメッセージを送信する場合は、開封率の向上のために、メールの下部ではなく上部にこのピクセルを移動することを検討してください。そうしないと、トラッキングピクセルが一部のメールプロバイダーによってカットされる可能性があります。
...
ターゲティングコンテキストを使用して、（オーディエンス定義画面で）電子メールのターゲティングと（HTML コンテンツエディターでパーソナライゼーションフィールドを定義する）パーソナライゼーションに使用する一連のテーブルを定義します。
...
* **[!UICONTROL Typology]**：送信の前に、コンテンツと設定を検証するためのメッセージを準備する必要があります。 準備段階中に適用される検証ルールは、**タイポロジ**&#x200B;内に定義されています。 例えば、メールの場合、件名、URL、画像などを確認して準備します。このフィールドで適用するタイポロジを選択します。
...
* **[!UICONTROL Compute the label during delivery preparation]**：パーソナライゼーションフィールド、コンテンツブロック、動的テキストを使用して、メッセージ準備段階のメールのラベル値を計算するには、このオプションを使用します。
...
* **[!UICONTROL Save SQL queries in the log]**：準備フェーズでSQL クエリログをジャーナルに追加するには、このオプションを使用します。

#### 配達確認設定 {#proof-settings}

この節では、プルーフメッセージの件名で使用するデフォルトのプレフィックスを設定できます。 配達確認について詳しくは、[この節](../../sending/using/sending-proofs.md)を参照してください。

### メール SMTP パラメーターのリスト {#list-of-email-smtp-parameters}
...
* **[!UICONTROL Additional SMTP headers]**：このオプションを使用すると、追加の SMTP ヘッダーをメッセージに追加できます。 **[!UICONTROL Headers]** フィールドに入力されたスクリプトは、行ごとに&#x200B;**name:value**&#x200B;の形式で1つのヘッダーを参照する必要があります。 値は必要に応じて自動的にエンコードされます。

  >[!IMPORTANT]
  >
  >スクリプトを追加すると、挿入する SMTP ヘッダーを追加できます。これは高度な知識を持つユーザー向けに用意されています。 スクリプトの構文は、このコンテンツタイプの要件を満たしている必要があります（不要なスペースや空行を含まないなど）。

  2024年6月1日（PT）以降、Google と Yahoo! は、 送信者は、**ワンクリックのリスト登録解除**&#x200B;に準拠する必要があります。 Campaignでは、配信テンプレート用にこの機能をすぐに使用できます。

  テンプレート以外のすべてのメール配信に&#x200B;**One-Click List-Unsubscribe**&#x200B;を適用するには、**[!UICONTROL Copy headers from delivery templates]**&#x200B;のテクニカルワークフローを実行する必要があります。 [詳細情報](technical-workflows.md)

  >[!CAUTION]
  >
  >メールテンプレートの&#x200B;**[!UICONTROL Additional SMTP headers]**&#x200B;のヘッダー値を変更すると、GoogleおよびYahoo！の&#x200B;**ワンクリック リスト登録解除**&#x200B;要件に準拠できなくなる可能性があります。

### アクセス認証パラメーターのリスト {#list-of-access-authorization-parameters}

「**[!UICONTROL Access authorization]**」セクションでは、次のパラメーターについて説明します。

* **[!UICONTROL Organizational unit]** フィールドは、この電子メールへのアクセスを特定のユーザーに制限するために使用されます。 指定したユニットまたは親ユニットに関連付けられたユーザーは、このメールに対する読み取りと書き込みをおこなえるようになります。 子ユニットに関連付けられたユーザーは、このメールに対する読み取りのみおこなえるようになります。
...
## レガシー設定 {#legacy-settings}

最新バージョンのCampaignを実行している&#x200B;**NOT**&#x200B;の場合、以下に説明するパラメーターとUI セクションは引き続き適用されます。

### 再試行 {#legacy-retries}

[設定メニュー](#email-channel-parameters)および電子メールプロパティの[送信パラメーター](#retries-parameters)の&#x200B;**[!UICONTROL Retries]**&#x200B;設定は、送信を開始した日（**[!UICONTROL Number of retries]** / **[!UICONTROL Max. number of retries]**）から再試行を実行する必要がある回数と、再試行の間隔の最小遅延（**[!UICONTROL Retry period]**）を示しています。

再試行の数は、グローバルに変更できます（Adobeの技術管理者にお問い合わせください）。または、配信または配信テンプレートごとに変更できます。

デフォルトでは、5回の再試行が最初の日にスケジュールされ、最小の間隔は1時間で、1日の24時間に分散されます。 1日あたり1回の再試行は、その後および配信期限までプログラムされます。これは、**[!UICONTROL Configuration]** メニューの&#x200B;**[!UICONTROL Delivery parameters]** セクションまたは配信レベルの&#x200B;**[!UICONTROL Validity period]** セクションでグローバルに定義されます（以下の[配信期間](#legacy-delivery-duration) セクションを参照）。
...
[有効期限パラメーター](#validity-period-parameters) セクションの&#x200B;**[!UICONTROL Delivery duration]**&#x200B;または&#x200B;**[!UICONTROL Validity limit for sending messages]** パラメーターを使用して、メッセージを送信できる期間を指定します。

### メール処理ルール {#legacy-email-processing-rules}

**[!UICONTROL MX management]**、**[!UICONTROL Bounce mails]**、**[!UICONTROL Domain management]**&#x200B;のルールには、管理者が&#x200B;**[!UICONTROL Administration > Channels > Email > Email processing rules]** メニューからアクセスおよび変更できます。 [詳細情報](#email-processing-rules)

### バウンスメールの選定 {#legacy-bounce-mail-qualification}

様々なバウンス、および関連するエラーの種類と理由を一覧表示するには、左上の&#x200B;**Adobe** ロゴをクリックし、**[!UICONTROL Administration > Channels > Quarantines > Message qualification]**&#x200B;を選択します。

バウンスの適格性ステータスは次のとおりです。

* **[!UICONTROL To qualify]**: バウンスメールを修飾する必要があります。 プラットフォームの配信品質が正しく機能するように、配信品質チームが選定を行う必要があります。 バウンスメールが適格でない限り、バウンスメールはメール処理ルールのリストを充実させるために使用されません。
* **[!UICONTROL Keep]**: バウンスメールが修飾され、**配信品質の更新** ワークフローで使用され、既存のメール処理ルールと比較してリストを拡充します。
* **[!UICONTROL Ignore]**: バウンスメールは選定されましたが、**配信品質の更新** ワークフローでは使用されません。 クライアントインスタンスには送信されません。

>[!NOTE]
>
>ISP を利用できなくなった場合、Campaign を通じて送信されたメールは、誤ってバウンスとしてマークされます。 これを修正するには、バウンス選定条件を更新する必要があります。 [詳細情報](../../administration/using/update-bounce-qualification.md)。

<!--Bounces are qualified through the **[!UICONTROL Bounce mails]** processing rule. For more on accessing this rule, refer to this [section](#legacy-bounce-mail-qualification).-->

### 配信済みインジケーターレポート {#legacy-delivered-status-report}

各メッセージの&#x200B;**[!UICONTROL Summary]** ビューでは、ソフトバウンスとハードバウンスが報告されるため、配信の有効期間を通じて&#x200B;**[!UICONTROL Delivered]**&#x200B;の割合が徐々に増加します。

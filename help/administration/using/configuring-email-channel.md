...
実際にメッセージを送信せずにSMTP接続を介してメール送信をテストするには、**[!UICONTROL Enable SMTP test mode]** オプションを使用します。 配信は SMTP サーバーへの接続まで処理されますが、送信はされません。配信の受信者ごとに、Campaign は SMTP プロバイダーサーバーに接続し、「SMTP RCPT TO」コマンドを実行して、「SMTP DATA」コマンドの前に接続を閉じます。
...
**[!UICONTROL Retries]**&#x200B;設定メニュー[および電子メールプロパティの](#email-channel-parameters)送信パラメーター[の](#retries-parameters)設定は、送信を開始した日（**[!UICONTROL Number of retries]** / **[!UICONTROL Max. number of retries]**）から再試行を実行する必要がある回数と、再試行の間隔の最小遅延（**[!UICONTROL Retry period]**）を示しています。

再試行の数は、グローバルに変更できます（Adobeの技術管理者にお問い合わせください）。または、配信または配信テンプレートごとに変更できます。

デフォルトでは、5回の再試行が最初の日にスケジュールされ、最小の間隔は1時間で、1日の24時間に分散されます。 1日あたり1回の再試行は、その後および配信期限までプログラムされます。これは、**[!UICONTROL Delivery parameters]** メニューの&#x200B;**[!UICONTROL Configuration]** セクションまたは配信レベルの&#x200B;**[!UICONTROL Validity period]** セクションでグローバルに定義されます（以下の[配信期間](#legacy-delivery-duration) セクションを参照）。

### 配信期間 {#legacy-delivery-duration}

**[!UICONTROL Message delivery duration]**&#x200B;設定メニュー[の](#email-channel-parameters) パラメーターを使用して、一時的なエラーまたはソフトバウンスが発生した配信内のメッセージを再試行する時間枠を指定します。

**[!UICONTROL Delivery duration]**&#x200B;有効期限パラメーター&#x200B;**[!UICONTROL Validity limit for sending messages]** セクションの[または](#validity-period-parameters) パラメーターを使用して、メッセージを送信できる期間を指定します。

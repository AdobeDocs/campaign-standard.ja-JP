...
この節では、電子メールまたは電子メールテンプレートのプロパティ画面で使用できるパラメーターのリストについて説明します。
...
* 選択したBCCアドレスは、アドビのチームに提供され、アドビがお客様に合わせて設定する必要があります。
...
電子メールBCCは、電子メールテンプレートで、次の専 [用オプションを使用し](../../start/using/marketing-activity-templates.md)、アクティブ化されます。
...
1. このテンプレ **[!UICONTROL Archive emails]** ートに基づいて各配信に対して送信されたすべてのメッセージのコピーを保持する場合は、このチェックボックスをオンにします。

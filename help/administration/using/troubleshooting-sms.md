...
`BIND PDU` で使用できるフィールドは、前述の[各種 PDU の情報](../../administration/using/sms-protocol.md#information-pdu)の節を参照してください。通常は `address_range` にショートコードを追加しますが、プロバイダーの特別なサポートが必要です。 複数のショートコードを個別にルーティングする方法については、担当者にお問い合わせください。
...
* MTAを再開すると、一時的に問題が修正されます。 これは、不安定なコネクションがAdobe Campaign StandardでMTA スロットリングをトリガーし、MTAを再起動するとスロットリングがクリアされることを意味します。 原因が見つかるまで再び起こります。
...
* プロバイダーが`TCP FIN`または`TCP RST` パケットのいずれかを送信して接続を閉じる場合は、プロバイダーに詳細情報を依頼します。
...
* 接続が安定していることを確認します。 SMPP接続は、少なくとも1時間継続して稼働する必要があります。 [不安定な接続に関する問題](../../administration/using/troubleshooting-sms.md#issues-unstable-connection)の節を参照してください。
...
* Adobe Campaign が `DELIVER_SM_RESP PDU` に正常に応答したことを適切なタイミングで確認します。 Adobe Campaign Standardでは、処理ロジック全体が適用されていることを保証します。そうでない場合は、処理が失敗した理由を示すエラーメッセージがログに表示されることが保証されます。
...
* Adobe Campaign Standardの場合、`broadLog`と`broadLogExec`のテーブルが正しく同期されていることを確認してください。

すべてを修正しても、一部の無効なSRがまだプロバイダーのバッファーに残っている場合は、**無効なID確認カウント** オプションを使用してスキップできます。 このオプションは慎重に使用する必要があり、バッファがクリーンになった後、できるだけ早く 0 にリセットする必要があります。

## MOを処理する際の問題（およびブロックリスト/自動返信）{#issue-process-MO}
...
* 自動返信が有効な場合は、`SUBMIT_SM` がプロバイダーに送信されたことを確認します。 そうでない場合は、MTA ログにエラーメッセージが確実に見つかります。
...
* 電話番号の形式が、強制隔離テーブルと配信ログで完全に同じであることを確認します。  それ以外の場合で、国際電話番号形式のプラス接頭辞に問題がある場合は、この[節](../../administration/using/sms-protocol.md#automatic-reply)を参照してください。
...
問題の最も一般的な原因は、ワードプロセッサからのコピーペーストで、通常の文字を変更してタイポグラフィで修正するバージョンを変更します。スペースを改行しないスペースに変更し、二重引用符を開閉する引用符に変更し、マイナス記号を様々な種類のハイフンに変更します。
...
ネットワークキャプチャは必ずしも必要ではありません。通常は、詳細な SMPP メッセージで十分です。 ネットワーク キャプチャが必要かどうかを判断するのに役立つガイドラインを以下に示します。
...
1. **外部アカウント**&#x200B;で、「**ログファイル内の詳細SMPP トレースを有効にする**」を選択します。
1. 保存すると、コネクタはトレースを有効にして再接続されます。

**その場で有効にする**

Adobe Campaign Standard MTAには、トレース フィルターを即座に変更できるHTTP コントロール インターフェイスがあります。
POST呼び出しは、トレースを有効/無効にできます。 SMPP トレースを有効にするURLの例：

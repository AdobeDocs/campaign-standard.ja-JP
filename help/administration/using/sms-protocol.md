...
>[!NOTE]
>
>**SMSコネクタのプロトコルと、Adobe Campaign Classicの設定**&#x200B;は、この[ページ](https://experienceleague.adobe.com/docs/campaign-classic/using/sending-messages/sending-messages-on-mobiles/sms-protocol.htmln#sending-messages)にあります。
>
...
Adobe Campaign Standardは、SMPPプロトコルバージョン3.4をサポートしています。これは、SMSをプロバイダ(SMSC)に送信し、SMSを受信し、領収書も受信できる広範なプロトコルです。 詳しくは、[SMPP ドキュメント](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)を参照してください。
...
**エンコードの問題が発生した場合は、必ず SMSC プロバイダーに連絡する**&#x200B;必要があります。サポートするエンコーディングと、SMSCプロバイダーの技術プラットフォームの制限によって適用される可能性のある特別な規則に関する正確な知識を持つのは、SMSCプロバイダーだけです。
...
これらの設定は、**送信機+受信機**&#x200B;モードの場合に受信機に適用されます。 トランスミッター部分と同様に機能します。詳しくは、上記を参照してください。
...
最終的なアーキテクチャで適切にベンチマークを付けない限り、正確なスループットをこの数を超えることは不可能なので、通常、この設定を1000未満に保つことをお勧めします。 1000を超えるスループットが必要な場合は、プロバイダにお問い合わせください。 ただし、接続数を増やして 1000 MT を超える方が良い場合もあります。
...
>[!NOTE]
>
>21.1リリース以降、複数のオプションパラメーターを追加できるようになりました。 詳しくは、[この節](../../administration/using/sms-protocol.md#automatic-reply-tlv)を参照してください。
...
### 自動応答オプションパラメータ(TLV) {#automatic-reply-tlv}

21.1リリース以降、自動応答MTにオプションのパラメータを追加できます。 [SMPP仕様](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)（ページ131）のセクション5.3で説明されているように、応答の`SUBMIT_SM PDU`にオプションのTLVパラメーターとして追加されます。
...
有効期間は、`SUBMIT_SM PDU` の `validity_period` フィールドに送信されます。日付は、常に絶対UTC時間形式で表示されます（日付フィールドは「00+」で終わります）。

#### SMPPオプションパラメータ(TLV) {#smpp-optional-parameters}

21.1リリース以降、この配信用に送信されるすべてのMTに、複数のオプションパラメータを追加できます。 これらのオプションのパラメータは、[SMPP仕様](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)（ページ131）のセクション5.3で説明されているように、応答の`SUBMIT_SM PDU`に追加されます。

表の各行は、オプションのパラメーターを表します。

* **パラメータ**:パラメーターの説明。プロバイダーに送信されません。
* **タグID**:オプションのパラメーターのタグ。0x1234形式の有効な16進数である必要があります。 無効な値は、配信の準備エラーになります。
* **値**:オプションのフィールドの値。プロバイダーに送信する場合は、UTF-8としてエンコードされます。 エンコード形式を変更できない。バイナリ値を送信したり、UTF-16やGSM7などの異なるエンコーディングを使用したりすることはできません。

任意のパラメーターの&#x200B;**タグID**&#x200B;が、外部アカウントで定義されている&#x200B;**サービスタグID**&#x200B;と同じ場合は、この表で定義されている値が使用されます。
...
他のインスタンスがこのアカウントに接続していないことを確認します。 特に、ステージ環境がアカウントに接続していないことを確認してください。 一部のプロバイダーではこれをサポートしていますが、Adobe Campaign側とプロバイダーのプラットフォームの両方で、非常に固有の設定が必要です。

同じプロバイダーに接続する同じAdobe Campaignインスタンスに複数のアカウントが必要な場合は、プロバイダーに問い合わせて、これらのアカウント間の接続を実際に区別するかどうかを確認してください。 複数のアカウントに同じログインを割り当てる場合は、追加の設定が必要です。

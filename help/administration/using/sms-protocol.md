...
>[!NOTE]
>
>Adobe Campaign Classicの&#x200B;**SMS コネクタプロトコルと設定**&#x200B;は、この[ ページ ](https://experienceleague.adobe.com/docs/campaign-classic/using/sending-messages/sending-messages-on-mobiles/sms-protocol.html)にあります。
>
...
このドキュメントでは、Adobe CampaignとSMPP プロバイダー間の接続の設定について説明します。
...
Adobe Campaign Standardは、SMPP プロトコルバージョン 3.4をサポートしています。 これは、プロバイダー（SMSC）にSMSを送信し、SMSとレシートを受信できる広範なプロトコルです。 詳しくは、[SMPP ドキュメント](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)を参照してください。
...
* **トランシーバ （abor TRX）**: メッセージの送受信には、1つのTCP接続が使用されます。

>[!NOTE]
>
>TRXは、接続数を減らし、障害が発生した場合の接続復旧を簡素化するため、Adobe Campaign Standardに適しています。
...
Adobe Campaign Standardでは、MTとSRの紐付けはMTAにネイティブであるため、専用のSMS プロセスはありません。
...
この PDU は、切断前にシステムから送信する必要があります。 接続を閉じるには、一致する`UNBIND_RESP PDU`を待つ必要があります。
...
Adobe Campaignは、一部の設定を調整して、`receipted_message_id` オプションフィールドのメッセージ IDを読み取ることができます。

#### DELIVER_SM_RESP {#deliver-sm-resp}

この PDU は、SR と MO を確認するために Adobe Campaign から送信されます。

Adobe Campaign Standardは、すべての処理手順が正常に完了した場合にのみ`DELIVER_SM_RESP`を送信します。 これにより、処理エラーのリスクが残っている間は、SRまたはMOが確認されないことが保証されます。
...
[SMPP プロトコル仕様](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) （ページ 167）の付録B セクションで推奨されていますが、実際のエラーコードやその意味は記載されていません。
...
### Adobe Campaign Standard拡張ジェネリック SMPPでのSR処理の例 {#sr-processing}
...
**エンコードの問題が発生した場合は、必ず SMSC プロバイダーに連絡する**&#x200B;必要があります。 SMSC プロバイダーのみが、サポートするエンコーディングと、技術的プラットフォームの制限により適用される可能性のある特別なルールに関する正確な知識を持っています。
...
* どの文字がどのエンコーディングに属するのかを把握しておくようにします。 GSM7は、発音区別符号（アクセント）を完全にはサポートしていません。 フランス語の場合、é と è が GSM7 に含まれますが、ê、â、ï は含まれません。 スペイン語も同じです。
...
接続を&#x200B;**トランシーバ** モードまたは分離&#x200B;**トランスミッタ+レシーバ** モードに設定します。 分離された&#x200B;**送信機+受信機** モードに切り替えると、**SMPP接続モード** セクションの設定が送信機に適用され、**受信者接続設定** セクションの設定が受信者接続に適用されます。「**受信者に異なるパラメーターを使用**」チェックボックスをオンにした場合のみ。
...
Adobe Campaign Standardでは、SMS スレッドとMTA プロセスごとの接続数を定義します。
MTA プロセスの数はデプロイメントによって決まります。通常、2つのMTAと1つのスレッドがあります。 スレッドの数は、smppConnectorThreads設定を使用して、config-instance.xml ファイルで変更できます。 通常、コンテナごとに1つのMTA プロセスとMTA プロセスごとに1つのスレッドがあります。

Adobe Campaign Standardの合計接続数の式：

* **合計接続=同時接続* スレッド数* MTA数**

同時接続は外部アカウントで設定され、スレッドの数はconfig-instance.xml ファイル（smppConnectorThreads）で設定され、MTAの数は外部アカウントで制限できます。

分離された&#x200B;**送信機/受信機** モードでは、上記の接続数は&#x200B;**送信機/受信機** ペアの数を表します。つまり、合計の接続数が2倍になります。
...
#### Source TON/NPI、宛先TON/NPI {#ton-npi}
...
最終的なアーキテクチャで適切にベンチマークされていない限り、この数値を超える正確なスループットを保証することは不可能であるため、通常、この設定を1000未満に保つことをお勧めします。 スループットが1000を超える場合は、プロバイダーにお問い合わせください。 ただし、接続数を 1000 MT 以上に増やした方が良い場合もあります。
...
>[!NOTE]
>
>21.1 リリースでは、複数のオプションパラメーターを追加できるようになりました。 詳しくは、[この節](../../administration/using/sms-protocol.md#automatic-reply-tlv)を参照してください。
...
### 自動返信オプションのパラメーター（TLV） {#automatic-reply-tlv}

21.1 リリースでは、自動返信MTにオプションのパラメーターを追加できます。 これらは、[SMPP仕様](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) （ページ 131）のセクション 5.3で説明されているように、返信の`SUBMIT_SM PDU`にオプションのTLV パラメーターとして追加されます。

オプションのパラメーターについて詳しくは、この[ セクション ](../../administration/using/sms-protocol.md#smpp-optional-parameters)を参照してください。
...
有効期間は、`SUBMIT_SM PDU` の `validity_period` フィールドに送信されます。 日付は常に絶対UTC時間形式でフォーマットされます（日付フィールドは「00+」で終わります）。

#### SMPP オプションのパラメーター（TLV） {#smpp-optional-parameters}

21.1 リリースでは、この配信のために送信されたすべてのMTに複数のオプションのパラメーターを追加できます。 これらのオプションのパラメーターは、[SMPP仕様](https://smpp.org/SMPP_v3_4_Issue1_2.pdf) （ページ 131）のセクション 5.3で説明されているように、返信の`SUBMIT_SM PDU`に追加されます。

テーブルの各行は、オプションのパラメーターを表します。

* **パラメーター**: パラメーターの説明。 プロバイダに送信されません。
* **タグ ID**: オプションのパラメーターのタグ。 0x1234の形式を使用して、有効な16進数である必要があります。 値が無効な場合、配信準備エラーが発生します。
* **値**: オプション フィールドの値。 プロバイダーに送信される際にUTF-8としてエンコードされます。 エンコーディング形式を変更できません。バイナリ値を送信したり、UTF-16やGSM7などの異なるエンコーディングを使用したりすることはできません。

任意のパラメーターに、外部アカウントで定義された&#x200B;**サービスタグ Id**&#x200B;と同じ&#x200B;**タグ Id**&#x200B;が含まれている場合、このテーブルで定義された値が優先されます。
...
ここで注意すべき最も重要なことは、複数のSMPP コネクタスレッドがあることです。 これらのスレッドはすべて同じで、同じ設定を共有します。 そのため、接続数には常にスレッド数が掛けられます。

設定ファイルを変更する必要があるため、お客様はスレッドの数を変更できません。

### SMPP コネクタの動作の説明 {#behavior-smpp-connector}

#### MT、SR、broadLog エントリの照合 {#matching-mt-sr}

Adobe Campaignでは、メッセージはブロードログエントリです。 Adobe Campaign Standardでは、外部コネクタは作業中のブロードログテーブル `nmsBroadLogExec`についてのみ知る必要があります。 ワークフローは、ブロードログエントリを特定のターゲットディメンション（nmsBroadLogXXX）にコピーする責任があります。

残念ながら、SMPPはメッセージと一緒にIDを送信することを許可していません。プロバイダーはMT IDを各MTに与え、次に同じIDを持つ1つ以上のSRを提供します。

プロバイダーによって指定されたIDは、`nmsBroadLogExec` テーブルの`sProviderId`列に保存されます。 SRは常にMTが正常に送信され、確認された後に到着しますが、時には注文から到着することができます、Adobe Campaignでは優れたSRとして知られています。 処理スレッドは、完全な情報が届くまで、これらのSRを一時的にRAMに保存します。

MTが確認されると（`SUBMIT_SM_RESP`）、`sProviderId`はすぐにデータベースで更新されます。

各SRは、SMPP処理スレッドによって個別に処理されます。 このプロセスは疑似同期です。外部から同期と見なされますが、イベント駆動型の実装を使用して内部に実装されます。 SRが確認されるのは、ブロードログが正常に更新された場合のみです。エラーが発生した場合は、SRが拒否されます。

各SRに適用されるプロセスは次のとおりです。

* SRのIDは、正規表現を使用して抽出されます。
* IDは`nmsBroadLogExec:sProviderId`で検索されます。
* ステータス + エラーコードは、正規表現を使用してSRから抽出されます。
* ブロードログメッセージのメカニズムは、エラーを検証し、ブロードログメッセージ IDを見つけるために使用されます。
* 上記のすべての情報でブロードログが更新されます。
* SRが承認されます。

上記の手順を確認するには、すべての手順が正しく適用されていることを手動で確認するために、**詳細SMPP トレースを有効にする**&#x200B;必要があります。 これは、Adobe Campaignが新しいSMPP プロバイダーに接続されるたびに必要になります。
...
他のインスタンスがこのアカウントに接続していないことを確認してください。 特に、ステージ環境がアカウントに接続されていないことを確認します。 一部のプロバイダーではサポートしていますが、Adobe Campaign側とプロバイダーのプラットフォームの両方で、非常に具体的な設定が必要です。

同じAdobe Campaign インスタンスで同じプロバイダーに接続する複数のアカウントが必要な場合は、プロバイダーに連絡して、これらのアカウント間の接続を実際に区別するようにしてください。 複数のアカウントに同じログインを割り当てる場合は、追加の設定が必要です。
...
`BIND_* PDU`が多すぎないことを確認してください。 数が多すぎる場合は、接続が不安定であることを示している可能性があります。 詳しくは、[不安定な接続の問題](../../administration/using/sms-protocol.md#issues-unstable-connection)の節を参照してください。
...
### プロバイダーに問い合わせると、問題ないようです {#provider}

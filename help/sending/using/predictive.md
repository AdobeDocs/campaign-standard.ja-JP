...
Adobe Campaign Standardオファーには、次の2つの新しい機械学習モデルがあります。 **予測送信時間の最適化** 、 **予測エンゲージメントスコアリング**。 この2つのモデルをまとめてJeurney AIと呼びます。これは、より良いカスタマージャーニーを設計および提供するための、機械学習モデルのクラスです。
...
>[!CAUTION]
>この機能は、製品の一部として初期状態では使用できません。 導入には、アドビのコンサルティングが関与している必要があります。 詳しくは、アドビの担当者にお問い合わせください。
>
...
* 1日で最も良い時間帯を計算し、次の7日間の電子メールを1時間間隔で送信します(例： 9:00 am、10:00 am、11:00 am)
* モデルは、次の7日間に電子メールを送信するのに最適な時間を示します。
...
* 16個のフィールドが表示されます（曜日は14個、週全体は2個）。
   ...
   * 週全体の開封を最適化するために電子メールを送信するのに最適な時間（0 ～ 23の値）

...
* **[!UICONTROL Unreachable]** （ソフト/ハードタイプ）:メッセージ配信チェーンでエラーが発生しました（SMTPリレーの問題、ドメインが一時的に到達不能な場合など）。 プロバイダーから返されたエラーによると、アドレスは検疫に直接送信されるか、または配信は、検疫ステータスを正当化するエラーがキャンペーンで受信されるか、エラー数が5に達するまで再試行されます。

...
また、アドレスが隔離された場合やプロファイルが検ブロックリストに加える疫された場合は、配信の準備中にメッセージを除外することもできます。 除外されたメッセージは、配信ダッシュボードの「**[!UICONTROL Exclusion logs]**」タブに表示されます（[こちらの節](../../sending/using/monitoring-a-delivery.md#exclusion-logs)を参照）。
...
| **[!UICONTROL Error ignored]** | 無視 | このアドレスは許可されており、いつでも電子メールが送信されます。 |
| **[!UICONTROL Denylisted address]** | ハード | 送信時にアドレスがブロックリストに追加されました。 |

...
* **[!UICONTROL Large modal]**：このレイアウトは、大きなアラートスタイルのウィンドウに表示されます。アプリケーションは背景で表示されます。

  メディア（画像、ビデオ）、テキストおよびボタンのコンポーネントをサポートします。

* **[!UICONTROL Small modal]**：このレイアウトは、小さなアラートタイプのウィンドウとして表示されます。アプリケーションは背景で表示されます。
...
ローカル通知は、特定の時間に、またはイベントに応じて、アプリでのみトリガーできます。 ユーザーがインターネットにアクセスしなくても、アプリで何かが起きていることが通知されます。
ローカル通知の追跡方法については、[こちら](../../administration/using/local-tracking.md)を参照してください。

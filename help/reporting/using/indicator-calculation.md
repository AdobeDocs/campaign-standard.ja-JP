---
title: インジケーターの計算
seo-title: インジケーターの計算
description: インジケーターの計算
seo-description: 各指標の数式のリストを使用して、レポートの結果を把握します。
...
以下の表に、配信タイプに応じて異なるレポートおよびその計算数式で使用されるインジケーターの一覧を示します。
...
<table> 
 <thead> 
  <tr> 
   ...
   <th> <strong>インジケーター計算数式</strong><br /> </th> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> @uniqueimpressions<br /> </td> 
   <td> @unique(@count(status=view))<br /> </td> 
   <td> <span class="uicontrol">キャンペーンプロファイル（inAppProfile）</span> テンプレートに基づくTargetユーザーの場合、user= Recipient ID。<br /> モバイル <span class="uicontrol">アプリ（InAppBroadcast）のすべてのユーザー</span> 、 <span class="uicontrol">およびモバイルプロファイル（inApp）</span> テンプレートに基づくターゲットユーザー、user= MC IDまたは同等のユーザーをターゲットにして、ユーザー、モバイルアプリケーションおよびデバイスの一意の組み合わせを表します。<br /> </td> 
  </tr> 
  <tr> 

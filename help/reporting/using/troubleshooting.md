...
ユニーク開封数を把握するには、値3を与える&#x200B;**[!UICONTROL Unique Opens]**&#x200B;の行数を合計する必要があります。 ただし、メールのターゲットとなったプロファイルは 2 つだけなので、開封率は 150％です。

パーセンテージが100を超えないようにするには、**[!UICONTROL Unique Opens]**&#x200B;の定義は、開かれた一意のブロードログの数に維持されます。 この場合、P1 が 1 日目と 2 日目にメールを開いても、ユニーク開封数は 1 のままになります。
...
これは、**[!UICONTROL Open]** アクションを追跡できない場合でも、開封数を追跡するために動的レポートでヒューリスティクスが使用されていることが原因である可能性があります。

例えば、ユーザーがクライアントで画像を無効にし、電子メール内のリンクをクリックした場合、**[!UICONTROL Open]**&#x200B;はデータベースによって追跡されない可能性がありますが、**[!UICONTROL Click]**&#x200B;は追跡されます。

したがって、**[!UICONTROL Open]** トラッキングログのカウントは、データベース内で同じカウントを持たない可能性があります。
...
**[!UICONTROL Conditional formatting]**&#x200B;をカスタムに変更すると、値が上限に達すると、セルがグリーンになります。 一方、値が下限に達すると赤色になります。
...
* **[!UICONTROL Transactional Delivery]** ディメンションをレポートにドラッグ&amp;ドロップすると、結果として値&#x200B;**N/A**&#x200B;が表示される場合があります。 この値が表示されるのは、配信がトランザクションでない場合であっても、動的レポートがすべての配信を取得するためです。 これは、**[!UICONTROL Delivery]** ディメンションをレポートにドラッグ&amp;ドロップする場合にも発生する可能性がありますが、この場合、**N/A**&#x200B;値はトランザクション配信を表します。
* ディメンションに関連しない指標と共にディメンションが使用される場合。 以下の例では、この配信で&#x200B;**[!UICONTROL Click]** カウントが0に設定されていても、**[!UICONTROL Tracking URL]** ディメンションに分類が追加されています。
...
* Target マッピングを読み込む代わりに、Adobe Campaign Standardで直接作成すると、レポートのエンリッチメントが自動的に作成されます。

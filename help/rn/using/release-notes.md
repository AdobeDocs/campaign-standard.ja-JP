 ...
 <tbody> 
  <tr> 
   ...
   <ul>
    ...
    <li><p>プライバシーリクエストを作成する際に、次の規則のタイプを選択できるようになりました。GDPRまたはCCPA。<p>
      <p><strong>注意</strong>:この新しいフィールドは必須です。 アクセスおよび削除のリクエストにキャンペーンプライバシーAPIを使用する場合は、ペイロードに含める必要があります。 See the <a href="https://docs.campaign.adobe.com/doc/standard/en/api/ACS_API.html#privacy-management">API documentation</a>.</p></li>
...
* 電子メールデザイナーを使用してリンクを編集する場合、「下線」リンクオプションを使用で **きるようにな** りました。 また、 **Target** プロパティが追加され、デフォルト値が「なし」に設定されて **います**。 [詳細を表示](../../designing/using/styles.md#about-styling-links)
...
* キャンペーン標準APIで、ペイロードを使用してプロファイルの地理的単位と組織単位を更新できるようになりました。 詳しくは、[詳細ドキュメント](https://final-docs.campaign.adobe.com/doc/standard/en/api/ACS_API.html)を参照してください。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>このリリースには、Adobe Experience Platform LaunchおよびMobile SDKのAdobe Campaign Standard用GAバージョンのAndroidおよびiOS拡張機能とAdobe Campaignの統合が含まれています。 これらの拡張機能は、プッシュメッセージ、アプリ内メッセージおよびモバイルアプリプロファイルの更新をサポートします。</p><p> 詳しくは、<a href="../../administration/using/about-typology-rules.md#typology-rules">詳細ドキュメント</a>を参照してください。</p> </td> 
  </tr> 
  <tr> 
...
* モバイルビューで編集する場合、モバイルディスプレイでのみ特定のコンポーネントを非表示にして、スペースを効果的に使用できます。
   ...
   詳しくは、[詳細ドキュメント](../../designing/using/styles.md#switching-to-mobile-view)を参照してください。
...
* すべてのチャネルからのメッセージ送信が同時にトリガされた場合に、1つのチャネルに対して送信が低いと、他のチャネルからのメッセージ送信が遅くなる問題を修正しました。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>電子メールコンテンツの編集時に専用のモバイルビューに切り替えることができるようになりました。 これにより、余白の調整、フォントサイズの小ささ、背景色の変化など、モバイル表示用のすべてのスタイルオプションを個別に編集して、電子メールのレスポンシブデザインを微調整できます。</p><p> 詳しくは、<a href="../../designing/using/styles.md#switching-to-mobile-view">詳細ドキュメント</a>を参照してください。</p> </td> 
  </tr> 
  <tr> 

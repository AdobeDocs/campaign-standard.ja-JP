---
title: リリースノート 2018
description: このページでは、2018年リリースのAdobe Campaign Standardをすべてリストしています。
...
2018年のAdobe Campaign Standardの特定リリースをお探しですか？
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> モバイルアプリ用のAdobe Launch統合（ベータ版）<br /> </td> 
   <td> Adobe LaunchのAdobe Campaignとの統合により、Mobile SDK V5を使用したキャンペーンでのモバイルアプリプロパティアクティベーションのプロセスが簡素化および自動化されました。<br /> 詳しくは、 <a href="https://helpx.adobe.com/campaign/kb/configuring-app-sdk.html">詳細なドキュメントを参照してください</a>。<br /> </td> 
  </tr> 
 </tbody> 
</table>
...
* キャンペーンでモバイルアプリケーションを設定する際に、iOS証明書とAndroidサーバーキーが正常にアップロードされたかどうかと、有効期限がわかるようになりました。
   ...
   これについて詳しくは、 [SDK V4](https://helpx.adobe.com/campaign/kb/configuring-app-sdkv4.html) および [SDK V5を使用したモバイルアプリケーションの設定方法に関する詳細なドキュメントを参照してください](https://helpx.adobe.com/campaign/kb/configuring-app-sdk.html)。

* Targetのプロパティを定義する際にモバイルアプリを選択することで、特定のモバイルアプリにキャンペーンユーザーを設定できます。 この機能は、プッシュメッセージとアプリ内メッセージの両方のチャネルに対して使用されます。
...
* アドビのAnalyticsレポートとAdobe Campaignレポートでデータに相違がある問題を修正しました。 (CAMP-25393)
...
* SMSメッセージが自動応答で登録解除された場合でも、ブロックリスト上の受信者に配信され続ける問題を修正しました。 (CAMP-27128)
...
* レポートの共有タイムアウトが1分から4分に増加し、タイムアウトエラーを回避しました。
...
* 「オーディエンスを保存」アクティビティで、Adobe Experience Cloudのアプリケーションサブスクリプションを共有できないエラーを修正しました。
...
* Adobe Targetから動的な画像を含めると電子メールが送信されない可能性がある問題を修正しました。(CAMP-24848)
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> プライバシーコアサービスの統合により、1回のJSON API呼び出しで、複数のソリューションのコンテキストでGDPRリクエストを自動化できます。 <br /> プライバシー・コア・サービスからすべてのExperience Cloud・ソリューションに送られるGDPR要求は、キャンペーンによって自動的に処理されるようになりました。 <br /> 詳しくは、 <a href="https://docs.campaign.adobe.com/doc/standard/getting_started/en/ACS_GDPR.html">詳細なドキュメントを参照してください</a>。<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td> カスタムプロファイルデータとの動的なレポート<br /> </td> 
...
* 「nextProcessingDate」フィールドがワークフローリソースに追加されます。 このフィールドはREST API呼び出しでのみ表示され、次の処理日のワークフローを視覚化できます。
...
* 配信を送信した後にリアルタイムで **[!UICONTROL Delivered]****[!UICONTROL Bounces + Errors]** またはKPIを更新できるオプションが追加されました。 プロバイダから受け取ったSR（ステータスレポート）から直接再計算されます。
...
* Adobe CampaignクラシックからStandardにレプリケートされた受信者のデータへのアクセスおよびクエリの設定ができない問題を修正しました。
...
* 国際文字がアドビのAnalyticsに正しく送信されない問題を修正しました。

...
* [ユースケース：強化されたフィールドを含むメールの送信](../../automating/using/sending-email-enriched-fields.md)
...
1. データの読み込み元のファイルが圧縮されて GZIP ファイル（.gz）になっている場合は、「**[!UICONTROL Add a pre-processing stage]**」フィールドで「**[!UICONTROL Decompression]**」オプションを選択します。 これにより、データを読み込む前にファイルを解凍できます。 このオプションは、ファイルがアクティビティの受信トランジションから送信されている場合にのみ使用できます。
   ...
   **[!UICONTROL Add a pre-processing stage]** フィールドでは、データベースに読み込む前にファイルを復号化することもできます。 暗号化されたファイルの操作方法について詳しくは、[この節](../../automating/using/managing-encrypted-data.md)を参照してください
...
* **[!UICONTROL Error processing]**：エラーが発生した場合の処理を定義します。
  ...
  >[!NOTE]
  >
  >「**[!UICONTROL Error processing]**」で対象となるのは、インポートしたファイルの値に関するエラーです。 例えば、誤ったデータ型（「整数」列の場合は「4」すべて文字）、許可された最大数より多くの文字を含む文字列、誤った区切り記号を含む日付などが発生しました。ただし、このオプションは、空の値管理によって生成されるエラーには関係ありません。

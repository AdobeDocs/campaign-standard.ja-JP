...
したがって、重複排除ではインバウンドトランジションを 1 つに絞ることをお勧めします。 これを行うには、和集合アクティビティ、積集合アクティビティなど、ターゲティングニーズに対応するアクティビティを使用して、異なるクエリを組み合わせることができます。例：
...
* [ユースケース：読み込まれたファイルからのデータの重複排除](../../automating/using/deduplicating-data-imported-file.md)
...
1. 提供されるリストに条件を追加して、「**[!UICONTROL Duplicate identification]**」条件を定義します。 電子メールアドレス、名、姓など、同じ値を使用して重複を識別できるフィールドや式を指定します。条件の順序を指定すると、最初に処理する条件を指定できます。

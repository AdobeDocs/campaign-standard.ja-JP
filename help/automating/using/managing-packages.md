...
>[!IMPORTANT]
>
>書き出されるリソースにデフォルトの ID が含まれている場合、パッケージの書き出しは許可されません。したがって、書き出し可能なリソースの ID は、Adobe Campaign Standard で標準として提供されているテンプレートとは異なる名前に変更する必要があります。例えば、テストプロファイルを書き出すには、「SDM」または「sdm」を含む ID は使用しないでください。
>
...
>[!CAUTION]
>
>書き出されたリソースにデフォルトのままの ID が含まれている場合、パッケージの書き出しはできません。したがって、書き出し可能なリソースの ID は、Adobe Campaign Standard で標準として提供されているテンプレートとは異なる名前に変更する必要があります。例えば、テストプロファイルを書き出すには、「SDM」または「sdm」を含む ID は使用しないでください。

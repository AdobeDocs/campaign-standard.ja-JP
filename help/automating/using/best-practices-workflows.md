...
デフォルトでは、20 個を超えるアクティブなワークフローを同時に実行しないことをお勧めします。その限度に達すると、パフォーマンスに影響を与えないようにワークフローはキューに入れられます。同様に、Adobeでは、時間の経過と共にワークフローの実行を広げることをお勧めします。
...
>[!NOTE]
>
>ワークフローを開始する前に、[!DNL Campaign Standard]は、ワークフローを実行するのに十分なシステム物理メモリがあるかどうかを確認します。 十分な空きメモリがない場合は、サーバーの負荷が下がり、システムメモリが増加するまで、ワークフローの実行が遅れることを示すメッセージが表示されます。
...
最大100個のアクティビティを1つのワークフローに使用することをお勧めします。 100を超えるアクティビティで、ワークフローを設計および設定する際に、パフォーマンスの問題が発生する場合があります。
...
パラメーターの名前と数が、ワークフローの呼び出し時に定義されたものと同じであることを確認します（[このページ](../../automating/using/defining-parameters-calling-workflow.md)を参照）。 また、パラメーターのタイプは、想定される値と一致する必要があります。

---
title: 外部 API
description: External API アクティビティは、HTTP API呼び出しを介して、外部システムからCampaign Standard ワークフローにデータを取り込みます。
audience: automating
...
>[!CAUTION]
>
>外部 API アクティビティは、キャンペーン全体のデータ（最新のオファーセット、最新のスコアなど）を取得するためのものであり、各プロファイルの特定の情報を取得するためのものではありません（大量のデータが転送される可能性があるため）。 プロファイル関連のデータ取得が必要な場合のレコメンデーションは、「[ファイル転送](../../automating/using/transfer-file.md)」アクティビティを使用することです。
...
このタブでは、接続エンドポイントを定義できます。 **[!UICONTROL URL]** フィールドを使用すると、Campaign Standardが通信する&#x200B;**HTTPS エンドポイント**&#x200B;を定義できます。

エンドポイントで必要な場合は、次の2種類の認証方法を使用できます。
...
例えば、列のラベルを変更したり、そのタイプ（文字列、整数、日付など）を選択したり、エラー処理を指定したりできます。
...
単純なテストエンドポイントで外部API機能をテストするには、Postman Echoを使用できます。https://docs.postman-echo.com。
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> WKF-560246 - HTTP ヘッダーキーが不正です (ヘッダーキー : '%s')。</td> 
 ...
 <tr> 
   <td> WKF-560248 - HTTP ヘッダーキーは許可されていません (ヘッダーキー : '%s')。</td> 
   <td> <p>HTTP ヘッダーキーが使用できません (ヘッダーキー : 「Accept」)。</p></td> 
  </tr> 
  <tr> 
   <td> WKF-560247 - HTTP ヘッダー値が不正です（ヘッダー値：'%s'）。</td> 
  ...
  <tr> 
   <td> WKF-560279 - 「HTTP ヘッダーキーは許可されていません (ヘッダーキー : '%s')。」</td> 

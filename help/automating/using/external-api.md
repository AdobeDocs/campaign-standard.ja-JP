...
>[!CAUTION]
>
>External APIアクティビティは、キャンペーン全体のデータ(最新のオファーセット、最新のスコアなど)を取得する目的で使用され、各プロファイルの特定の情報を取得する目的では使用されません。取得したデータ量が多い場合に限ります。 使用事例で必要な場合は、「ファイルを [転送](../../automating/using/transfer-file.md) 」アクティビティを使用することをお勧めします。
